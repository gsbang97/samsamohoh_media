...
def live_plot(ax:plt.Axes, left,right,xlim=None, ylim=None) -> None:
    """
    matplotlib를 사용하여 실시간으로 라인 플롯에 팔 각도 데이터를 업데이트합니다.
    :param ax: matplotlib의 axis
    :param left: 왼쪽 팔 각도 데이터
    :param right: 오른쪽 팔 각도 데이터
    :param xlim: x축의 범위
    :param ylim: y축의 범위
    :return: None
    """
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if pose_landmarks is not None:
                ...
                print(landmarks['LEFT_ELBOW'], landmarks['RIGHT_ELBOW'])
                left_angle_lsit.append(left_angle)
                right_angle_lsit.append(right_angle)
                
                live_plot(ax, left_angle_lsit,right_angle_lsit,(0,200), (0,180))
                ax.axhline(MAX_ANGLE, color='g', linestyle='--')
                ax.axhline(MIN_ANGLE, color='g', linestyle='--')
            # 결과 이미지를 표시합니다.    
